...
resource "random_password" "vmss_runner_admin" {
  length      = 20
  min_special = 3
  min_lower   = 3
  min_upper   = 3
...
resource "azurerm_key_vault_secret" "vmss_runner_admin" {
  key_vault_id = module.keyvault_infra.key_vault_id
  name         = "vmss-runner-admin"
  value        = random_password.vmss_runner_admin.result
  tags         = {}
}
...
module "vmss_runner" {
  ...
  subnet = {
    name = module.vnet["runner"].subnet_names["runner"]
    id   = module.vnet["runner"].subnet_ids["runner"]
  }
  virtual_network = {
    name = module.vnet["runner"].virtual_network_name
    id   = module.vnet["runner"].virtual_network_id
  }

  os_flavor                       = "linux"
  linux_distribution_name         = "ubuntu2004"
  virtual_machine_size            = "Standard_B4ms"
  admin_username                  = azurerm_key_vault_secret.vmss_runner_admin.name
  admin_password                  = azurerm_key_vault_secret.vmss_runner_admin.value
  ...
  existing_network_security_group_id = module.vnet["runner"].network_security_group_ids["runner"]
...
//  version = "1"
  depends_on = [azurerm_key_vault_secret.vmss_runner_admin]
  ...
  subnet_id = module.vnet["runner"].subnet_ids["bastion"]
admin_password = azurerm_key_vault_secret.vmss_runner_admin.value
  admin_username = azurerm_key_vault_secret.vmss_runner_admin.name
  admin_ssh_key = null
