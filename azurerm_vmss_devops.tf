...
resource "random_password" "vmss_devops_admin" {
  length      = 20
  min_special = 3
  min_lower   = 3
  min_upper   = 3
...
resource "azurerm_key_vault_secret" "vmss_devops_admin" {
  key_vault_id = module.keyvault_infra.key_vault_id
  name         = "vmss-devops-admin"
  value        = random_password.vmss_devops_admin.result
  tags         = {}
}
...
module "vmss_devops" {
  ...
  subnet = {
    name = module.vnet["devops"].subnet_names["devops"]
    id   = module.vnet["devops"].subnet_ids["devops"]
  }
  virtual_network = {
    name = module.vnet["devops"].virtual_network_name
    id   = module.vnet["devops"].virtual_network_id
  }

  os_flavor                       = "linux"
  linux_distribution_name         = "ubuntu2004"
  virtual_machine_size            = "Standard_B4ms"
  admin_username                  = azurerm_key_vault_secret.vmss_devops_admin.name
  admin_password                  = azurerm_key_vault_secret.vmss_devops_admin.value
  ...
  existing_network_security_group_id = module.vnet["devops"].network_security_group_ids["devops"]
...
//  version = "1"
  depends_on = [azurerm_key_vault_secret.vmss_devops_admin]
  ...
  subnet_id = module.vnet["devops"].subnet_ids["bastion"]
admin_password = azurerm_key_vault_secret.vmss_devops_admin.value
  admin_username = azurerm_key_vault_secret.vmss_devops_admin.name
  admin_ssh_key = null
